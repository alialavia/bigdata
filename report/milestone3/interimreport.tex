...
\section{Data model}
The twitter streaming API provides data in JSON format. It outputs a tweet as a JSON object on a single line. We copied this data storage format to our permanent S3 storage. That is: we have a JSON object representing a tweet on each separate line in our data files.
...
The data processing platform we use is Apache Spark running on top of Amazon Elastic MapReduce (EMR). Initially we ran Spark on Amazon EC2 directly without using EMR. We used a custom Amazon Machine Image (AMI)\cite{amazonami}, which has Python2.7, Scikit-Learn \cite{scikit-learn} and NLTK \cite{nltk} preinstalled. It turned out that the default configuration did not work well, and the executed tasks had a failure rate of 25\% - 50\%. For this reason, we decided to try the Amazon Elastic MapReduce (EMR) platform. This platform has Spark pre-configured, which resulted in a 0\% failure rate for our tasks.

The data preprocessing is done by NLTK and Scikit-Learn. We use the default NLTK word tokenizer and stem each token using the Lancaster stemmer. Additionally we generate bigrams and trigrams of the text data. We use Scikit-Learn to generate sparse feature vectors by hashing each token to a feature. The final classification model is an SVM with $L_1$ regularization, trained using Spark's MLLib library\cite{mllib}.
