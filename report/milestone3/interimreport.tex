...
The Spark Map-Reduce framework naturally works with text data where each line represents a new element. By using functionality such as Spark's \texttt{textFile}, we can map each line of text to a specific function. Because each tweet resides on a different line of text, this means that we can map each individual tweet to a function. This makes using the data in a Map-Reduce context extremely easy.
